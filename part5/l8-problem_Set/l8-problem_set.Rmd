...
## Life expectancy versus median age across the years
 ?????????????????????????????????????????????
```{r}
library(ggplot2)

ggplot(gapminder, aes(x=median_age, y=life_expect)) +
  geom_point(aes(colour = year, size = population)) +
  scale_color_brewer(type = 'qual', palette = 'Set3')
```


## Life expectancy versus population_totals

```{r}
library(ggplot2)

ggplot(gapminder, aes(x=population, y=life_expect)) +
  geom_point(aes(colour = year)) +
  scale_color_brewer(type = 'qual', palette = 'Set3')
```