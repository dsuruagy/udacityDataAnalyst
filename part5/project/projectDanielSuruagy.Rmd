...
On this multivariate section, I will interconnect the variables which presents
the higher correlations on the bivariate section. Some characteristics of 
wines, like sweetness and group of pH (acidity) will be introduced in a try to
...
by(white_wines$density / white_wines$residual.sugar, white_wines$quality, summary)
```

I found nothing interesting in this plot, except that the pattern of values is
oposite to that shown in the density per quality plot.

```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(white_wines, aes(x = quality, y = alcohol/residual.sugar)) +
  geom_boxplot()
```

When looking for the alcohol/residual.sugar per quality plot, we can see the
same pattern saw in the above plot. That seems to be something in the values
that residual sugar have a relationshop with quality grades.
...
The quantity of residual sugar seems to have no influence on grades, however.
The only useful information presented here is that better grades are given to
wines with lower densities, no matter the residual sugar value.
...
would be lower and the amount of SO² also would be lower. But again, I could
not perceive relationship between higher quantities of SO² when there is a 
higher pH. I wonder why this winemaker was not worried about loss of 
...
Another weak corelation could be verified here. The darker colors of the higher
grades, appear on the lower left part of this plot, where the values for
chlorides and density are lower. Because there are fewer grades above 7 than 
lower, it is difficult to see a pattern here.
...
to see that drier wines are less dense than the sweets. No surprise, because
the residual.sugar and density is the strongest relationship as seen before.
...
No surprises here too, because it was seen that the best quality grades were
given to those wines that have presented where density is lower and alcohol 
quantity is greater. Is it possible to see the little darkening on the lower
right side of the above plot.


```{r echo=FALSE, Multivariate_Plots}
ggplot(white_wines, aes(y = alcohol, x = density)) +
  geom_line(aes(colour=quality), stat = 'summary', fun.y = median) +
  scale_color_brewer(type = "seq", palette = "YlGnBu") +
  xlim(0.987, 1)
```
